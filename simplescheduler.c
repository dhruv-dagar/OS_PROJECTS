...
// Global variable definitions (ONCE - remove duplicates!)
int NCPU = 1;
int TSLICE = 1000;
...
void scheduler_daemon() {
    while (!terminate_flag) {
        if (sem_wait(scheduler_sem) == -1) {
            ...
            perror("sem_wait scheduler_sem");
            break;
        }

        if (sem_wait(queue_sem) == -1) {
            perror("sem_wait queue_sem");
            continue;
        }
        ...
        for (int i = 0; i <= ready_queue->rear && scheduled < NCPU; i++) {
            if (ready_queue->processes[i].state == STATE_WAITING) {
                if (kill(ready_queue->processes[i].pid, SIGCONT) == -1) {
                    perror("kill SIGCONT");
                    continue;
                }

                // SET START TIME when process first starts running
                if (ready_queue->processes[i].start_time == 0) {
                    ...
                    for (int j = 0; j < shared_history->count; j++) {
                        if (shared_history->processes[j].pid == ready_queue->processes[i].pid) {
                            shared_history->processes[j].start_time = ready_queue->processes[i].start_time;
                            break;
                        }
                    }
                }
                
                ready_queue->processes[i].state = STATE_RUNNING;
                scheduled++;
            }
        }

        if (sem_post(queue_sem) == -1) {
            perror("sem_post queue_sem");
            continue;
        }
        ...
        for (int i = 0; i <= ready_queue->rear; i++) {
            if (ready_queue->processes[i].state == STATE_RUNNING) {
                if (kill(ready_queue->processes[i].pid, 0) == 0) {
                    // Process is still alive, stop it
                    if (kill(ready_queue->processes[i].pid, SIGSTOP) == -1) {
                        perror("kill SIGSTOP");
                        continue;
                    }
...
void add_to_history_and_queue(pid_t pid, const char *prog, int priority) {
    ...
    if (shared_history->count < MAX_HISTORY) {
        ...
        strncpy(p->command, prog, MAX_PROGRAM_NAME - 1);
        p->command[MAX_PROGRAM_NAME - 1] = '\0';
        p->pid = pid;
        p->priority = priority;
        p->submission_time = now;
        p->start_time = 0;
        p->completion_time = 0;
        p->wait_time = 0;
    ...
    if (ready_queue->rear < MAX_PROCESSES - 1) {
        Process proc;
        proc.pid = pid;
        strncpy(proc.command, prog, MAX_PROGRAM_NAME - 1);
        proc.command[MAX_PROGRAM_NAME - 1] = '\0';
        proc.priority = priority;
        proc.state = STATE_WAITING;
        proc.submission_time = now;
        proc.start_time = 0;
        proc.completion_time = 0;
        proc.wait_time = 0;

        int i = ready_queue->rear;
        while (i >= 0 && ready_queue->processes[i].priority < priority) {
            ready_queue->processes[i + 1] = ready_queue->processes[i];
            i--;
        }
        ready_queue->processes[i + 1] = proc;
        ready_queue->rear++;

        printf("Job submitted: PID=%d, Executable=%s, Priority=%d\n", pid, prog, priority);

        if (sem_post(scheduler_sem) == -1) {
            perror("sem_post scheduler_sem");
        }
    } else {
...
void cleanup() {
    ...
    for (int i = 0; i < shared_history->count; i++) {
        ...
        if (p->start_time)
            printf("    Start: %s", ctime(&p->start_time));
        if (p->completion_time)
            printf("    Completion: %s", ctime(&p->completion_time));
        printf("\n");
    }

    shmdt(ready_queue);
    shmctl(shmid_queue, IPC_RMID, NULL);
    shmdt(shared_history);
    shmctl(shmid_history, IPC_RMID, NULL);
